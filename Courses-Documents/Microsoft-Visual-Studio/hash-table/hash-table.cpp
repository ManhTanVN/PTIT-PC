...
// khai kieu du lieu mang thong tin can luu
typedef struct {
...
HashTable* create_table() {
    // Mang mot chieu co do dai  HASH_SIZE
...
void add_website(HashTable* table, char* website) {
    ...
    // tìm xem website dã t?n t?i chua thì tang bi?n count;
    while (current != NULL) {
    ...
    // Thêm website vào danh sách liên ket
...
int cmp(const void* a, const void* b) {
    ...
    if (wb->count != wa->count)
        return wb->count - wa->count; // sắp xếp theo thứ tự giảm dần
